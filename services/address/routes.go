...
var AuthorizeUser = middlewares.AuthorizeUser

func (h *Handler) RegisterRoutes(router *http.ServeMux) {
	router.HandleFunc(utils.RoutePath("GET", "/users/{id}/addresses"), Authenticate(AuthorizeUser(h.GetAllAddress)))
	router.HandleFunc(utils.RoutePath("GET", "/users/{id}/addresses/{addressId}"), Authenticate(AuthorizeUser(h.GetAddressById)))
	router.HandleFunc(utils.RoutePath("POST", "/users/{id}/addresses"), Authenticate(AuthorizeUser(h.GetAddressById)))
	router.HandleFunc(utils.RoutePath("PUT", "/users/{id}/addresses/{addressId}"), Authenticate(AuthorizeUser(h.GetAddressById)))
	router.HandleFunc(utils.RoutePath("DELETE", "/users/{id}/addresses/{addressId}"), Authenticate(AuthorizeUser(h.DeleteAddress)))
}
...
func (h *Handler) UpdateAddress(w http.ResponseWriter, r *http.Request) {
	...
	updatedAddr, err := h.store.UpdateAddress(*addressId, *userId,address, addrPayload)
	if err != nil {
		utils.WriteError(w, http.StatusInternalServerError, err)
		return
	}
